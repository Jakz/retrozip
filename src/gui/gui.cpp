...
#include "tbx/extra/portable-file-dialogs.h"
...
struct GUI
{
  ...
  void extractTo(const path& folder, std::vector<size_t>& indices)
  {
    setExtractionProgress(0.0f);
    nana::API::show_window(progress.form.handle(), true);

    std::thread([indices, folder, this]() {
      for (const auto& index : indices)
        extractNth(index, folder);
...
{  
  ...
  contextMenu.append("Extract here", [this](nana::menu::item_proxy item) {
    ...
    for (const auto& entry : selection)
      indices.push_back(entry.item);

    extractTo(filepath.parent(), indices);
    progress.form.modality();
  });

  contextMenu.append("Extract to...", [this](nana::menu::item_proxy) { 
    auto selection = table.selected();
    ...
    form.enabled(false);
    auto result = pfd::select_folder("Select Extraction Folder", pfd::path::home(), pfd::opt::force_path).result();
    form.enabled(true);

    if (!result.empty())
    {
      extractTo(result, indices);
    }
  });
